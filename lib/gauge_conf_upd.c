...
int metropolis_with_tracedef(Gauge_Conf *GC,
  ...
  GAUGE_GROUP *poly_dir, *poly_otherdir;
  ...
  err=posix_memalign((void*)&poly_dir, (size_t)DOUBLE_ALIGN, (size_t) param->d_size[i] * sizeof(GAUGE_GROUP));
   if(err!=0)
     {
  ...
  if(param->d_tracedef_dim > 2 || (param->d_tracedef_dim > 1 && NCOLOR>3))
    {
    fprintf(stderr, "Tracedef configurations with more than 2 compactified dimensions, or with 2 compactified dimensions and N_c != 3 have not been implemented yet. (%s, %d)\n", __FILE__, __LINE__);
    exit(EXIT_FAILURE);
    }
  for(hits=0; hits<numhits; hits++)
     {
         ...
         for(int t=0; t<param->d_size[i]; t++)
            {
               ...
               times_equal(&tmp2, &(GC->lattice[r][i]));               
               times_equal(&tmp2, &stap_cross_terms_dag[t]);
               rpart=NCOLOR*retr(&tmp2);
               ipart=NCOLOR*imtr(&tmp2);
